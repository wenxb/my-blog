...
export const getSongUrl = (id, level = "standard") => {
    ...
    const jsonString = JSON.stringify({id, level})
    const token = btoa(encryptor.encrypt(jsonString) || "")
...
export const getNormalSongUrl = async (id) => {
    const res = await getSongUrl(id, 'higher');
    // 检查是否有有效的响应数据
...
const converted = (data) => {
    let lines = [], trans = [], romans = []

    if (data.yrc) {
        lines = parseYrc(data.yrc)
        if (data.ytlrc) {
            trans = parseLrc(data.ytlrc)
        }
        if (data.yromalrc) {
            romans = parseLrc(data.yromalrc)
        }
        ...
        return lines.map((line, i, lines) => ({
            ...
            translatedLyric: trans[i]?.words?.[0]?.word ?? "",
            romanLyric: romans[i]?.words?.[0]?.word ?? "",
            isBG: false,
            isDuet: false,
        }));
    } else if (data.lrc) {
        ...
        if (data.tlyric) {
            trans = parseLrc(data.tlyric)
        }
        if (data.romalrc) {
            romans = parseLrc(data.romalrc)
        }
